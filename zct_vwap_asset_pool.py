...
**触轨池（唯一链路）**：上海时间 **每 2h**（偶数整点 :07，可配 `ZCT_TOUCH_POOL_CRON_HOURS`）对全宇宙做 **6h** walk-forward，
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        help="win+loss 须 ≤ 本值（默认 22；0=关闭上限）",
    )
    ap.add_argument(
        "--min-touch-share",
        type=float,
