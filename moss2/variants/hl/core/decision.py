...
from moss2.variants.hl.core.indicators import (
...
def compute_last_composite(
    df: pd.DataFrame,
    params: DecisionParams,
    regime: pd.Series,
) -> float:
    """最后一根已评分 K 线的加权 composite（非离散信号）。"""
    params.normalize_weights()
    n = len(df)
    start = max(params.slow_ma_period, 50)
    if n <= start:
        return 0.0
    cache: dict = {}
    return _composite_at_bar(df, params, regime, n - 1, cache)
...
def compute_signals(
    ...
    for i in range(max(params.slow_ma_period, 50), n):
        composite = _composite_at_bar(df, params, regime, i, cache)
