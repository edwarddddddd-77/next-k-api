...
运行时总开关（未设 env 时均为开启，仅显式 MOSS_QUANT_*=0 可关）：
  ENABLED, PAPER, SCHEDULER, DAILY_OPTIMIZE, APPLY_PROFILES, REFRESH, BOOTSTRAP,
  REAL_MODE, PAPER_SOURCE_OF_TRUTH, LLM, VERBOSE_LOG
寻优/纸面能力在下方常量或同名 env（*_ENABLED, default=True）中默认开启。
"""
...
MOSS_QUANT_OPTIMIZE_FULL_RISK_SLOTS = 3
MOSS_QUANT_OPTIMIZE_REDUCED_RISK_SCALE = 0.5
MOSS_QUANT_OPTIMIZE_TRAILING_FOR_TREND = True
...
# 网格结束后：70% 归因总结；30% 邻域精修验证（最多 N 轮早停）
MOSS_QUANT_OPTIMIZE_TUNING_DIAG_ENABLED = env_truthy(
    "MOSS_QUANT_OPTIMIZE_TUNING_DIAG_ENABLED", default=True
)
MOSS_QUANT_OPTIMIZE_LOCAL_REFINE_ENABLED = env_truthy(
    "MOSS_QUANT_OPTIMIZE_LOCAL_REFINE_ENABLED", default=True
)
MOSS_QUANT_OPTIMIZE_LOCAL_REFINE_MAX_ROUNDS = 3

# --- L3：最近 1500 根定模板+战术（与纸面 K 线同长）---
MOSS_QUANT_RECENT_PICK_ENABLED = env_truthy(
    "MOSS_QUANT_RECENT_PICK_ENABLED", default=True
)
MOSS_QUANT_RECENT_PICK_BARS = MOSS_QUANT_KLINE_LIMIT
MOSS_QUANT_RECENT_PICK_MIN_BARS = 400
MOSS_QUANT_RECENT_PICK_MIN_TRADES = 6
MOSS_QUANT_RECENT_PICK_TAIL_RATIO = 0.3
MOSS_QUANT_RECENT_PICK_TAIL_MIN_RETURN = -0.03
MOSS_QUANT_RECENT_PICK_REQUIRE_L1_OK = True
# False：L1 不可同步时仍跑 L3 供看板；同步与否由 L1 门禁决定，不因「无合格组合」否决 L1
MOSS_QUANT_RECENT_PICK_REQUIRE_SYNC = False
MOSS_QUANT_RECENT_PICK_REGIME_TEMPLATES = True
MOSS_QUANT_RECENT_PICK_MAX_COMBINATIONS = 48
MOSS_QUANT_RECENT_PICK_REFINE_ROUNDS = 2
MOSS_QUANT_RECENT_PICK_ENTRY_THRESHOLDS = (0.40, 0.44, 0.48, 0.52)
MOSS_QUANT_RECENT_PICK_SL_ATR_MULTS = (2.0, 2.5)
MOSS_QUANT_RECENT_PICK_TP_RR_RATIOS = (2.0, 2.5, 3.0)
MOSS_QUANT_RECENT_PICK_EXIT_THRESHOLDS = (0.10, 0.12, 0.14)
MOSS_QUANT_RECENT_PICK_MIN_PROFIT_FACTOR = 1.0
MOSS_QUANT_RECENT_PICK_SCORE_USE_PF = True

# 纸面扫描：训练窗多空胜率差 → 不对称开仓门槛
MOSS_QUANT_SIDE_BIAS_ADJUST_ENABLED = env_truthy(
    "MOSS_QUANT_SIDE_BIAS_ADJUST_ENABLED", default=True
)
MOSS_QUANT_SIDE_BIAS_WIN_GAP = 0.12
MOSS_QUANT_SIDE_BIAS_MIN_SIDE_TRADES = 3
MOSS_QUANT_SIDE_BIAS_SHORT_EXTRA_BUMP = 0.04
MOSS_QUANT_SIDE_BIAS_LONG_EXTRA_BUMP = 0.04

# 同步前要求 L3 尾段收益不低于 RECENT_PICK_TAIL_MIN_RETURN
MOSS_QUANT_SYNC_REQUIRE_RECENT_TAIL_OK = env_truthy(
    "MOSS_QUANT_SYNC_REQUIRE_RECENT_TAIL_OK", default=True
)

# --- 组合风控（纸面开仓前）---
MOSS_QUANT_PORTFOLIO_RISK_ENABLED = env_truthy(
    "MOSS_QUANT_PORTFOLIO_RISK_ENABLED", default=True
)
...
MOSS_QUANT_GATE_HARD_BLOCK = env_truthy("MOSS_QUANT_GATE_HARD_BLOCK", default=True)
...
# --- 纸面开仓质量（余量 + 多根确认）---
MOSS_QUANT_ENTRY_QUALITY_ENABLED = env_truthy(
    "MOSS_QUANT_ENTRY_QUALITY_ENABLED", default=True
)
MOSS_QUANT_ENTRY_MARGIN = float(os.getenv("MOSS_QUANT_ENTRY_MARGIN", "0.05") or 0.05)
MOSS_QUANT_ENTRY_CONFIRM_BARS = max(
    0, int(os.getenv("MOSS_QUANT_ENTRY_CONFIRM_BARS", "2") or 2)
)
MOSS_QUANT_ENTRY_MARGIN_ALIGNED_RELAX = float(
    os.getenv("MOSS_QUANT_ENTRY_MARGIN_ALIGNED_RELAX", "0.02") or 0.02
)

# --- 寻优验证：可达性 + gate 后子样本（对齐纸面）---
MOSS_QUANT_OPTIMIZE_REACHABLE_ENABLED = env_truthy(
    "MOSS_QUANT_OPTIMIZE_REACHABLE_ENABLED", default=True
)
MOSS_QUANT_OPTIMIZE_REACHABLE_REGIME_ALIGN = env_truthy(
    "MOSS_QUANT_OPTIMIZE_REACHABLE_REGIME_ALIGN", default=True
)
MOSS_QUANT_OPTIMIZE_REACHABLE_MIN_RATIO = float(
    os.getenv("MOSS_QUANT_OPTIMIZE_REACHABLE_MIN_RATIO", "0.01") or 0.01
)
MOSS_QUANT_OPTIMIZE_REACHABLE_FAIL_RATIO = float(
    os.getenv("MOSS_QUANT_OPTIMIZE_REACHABLE_FAIL_RATIO", "0") or 0
)
MOSS_QUANT_OPTIMIZE_REACHABLE_MIN_SUB_TRADES = 5
MOSS_QUANT_OPTIMIZE_REACHABLE_MIN_SUB_PF = float(
    os.getenv("MOSS_QUANT_OPTIMIZE_REACHABLE_MIN_SUB_PF", "1.0") or 1.0
)
MOSS_QUANT_OPTIMIZE_REACHABLE_FORWARD_BARS = max(
    1, int(os.getenv("MOSS_QUANT_OPTIMIZE_REACHABLE_FORWARD_BARS", "4") or 4)
)
MOSS_QUANT_OPTIMIZE_REACHABLE_PENALTY_SCALE = float(
    os.getenv("MOSS_QUANT_OPTIMIZE_REACHABLE_PENALTY_SCALE", "0.12") or 0.12
)
MOSS_QUANT_RECENT_PICK_MIN_REACHABLE_RATIO = float(
    os.getenv("MOSS_QUANT_RECENT_PICK_MIN_REACHABLE_RATIO", "0.008") or 0.008
)

# --- 同步门控 ---
MOSS_QUANT_SYNC_BLOCK_RECENT_LOSS_ENABLED = env_truthy(
    "MOSS_QUANT_SYNC_BLOCK_RECENT_LOSS_ENABLED", default=True
)
MOSS_QUANT_SYNC_BLOCK_LOSS_DAYS = 7
MOSS_QUANT_SYNC_BLOCK_LOSS_PCT = -0.05

# --- 日内门槛微调 ---
MOSS_QUANT_INTRADAY_ADJUST_ENABLED = env_truthy(
    "MOSS_QUANT_INTRADAY_ADJUST_ENABLED", default=True
)
...
MOSS_QUANT_REGIME_ALIGN_ADJUST_ENABLED = env_truthy(
    "MOSS_QUANT_REGIME_ALIGN_ADJUST_ENABLED", default=True
)
...
MOSS_QUANT_POOL_AUTO_DISABLE = env_truthy(
    "MOSS_QUANT_POOL_AUTO_DISABLE", default=True
)
MOSS_QUANT_POOL_AUTO_ENABLE = env_truthy(
    "MOSS_QUANT_POOL_AUTO_ENABLE", default=True
)
...
MOSS_QUANT_POOL_AUTO_DISABLE_ON_PAPER_LOSS = env_truthy(
    "MOSS_QUANT_POOL_AUTO_DISABLE_ON_PAPER_LOSS", default=True
)
