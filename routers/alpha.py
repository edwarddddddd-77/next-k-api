...
@router.get("/api/alpha/backtest")
async def get_alpha_backtest(
    refresh: bool = Query(False, description="强制重跑历史事件回测"),
):
    """
    上新早期窗口价量回测（币安合约 5m）。
    验证『开盘初期偏买盘』；不回溯巨鲸地址（免费接口做不到）。
    """
    from alpha_backtest import run_backtest

    if not refresh:
        try:
            return run_backtest(force_refresh=False)
        except Exception as e:
            logger.exception("alpha backtest read failed")
            raise HTTPException(status_code=502, detail=f"backtest_failed: {e}") from e

    allowed, wait = _refresh_cooldown.check_allow()
    if not allowed:
        raise HTTPException(status_code=429, detail=f"refresh_cooldown:{wait:.0f}s")
    try:
        data = run_backtest(force_refresh=True)
        _refresh_cooldown.mark_used()
        return data
    except Exception as e:
        logger.exception("alpha backtest failed")
        raise HTTPException(status_code=502, detail=f"backtest_failed: {e}") from e


@router.post("/api/alpha/backtest/refresh")
async def post_alpha_backtest_refresh():
    """强制重跑历史事件回测。"""
    from alpha_backtest import run_backtest
    ...
    try:
        ...
        logger.exception("alpha backtest refresh failed")
        raise HTTPException(status_code=502, detail=f"backtest_failed: {e}") from e
